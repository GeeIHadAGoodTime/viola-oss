...
import { useAuth } from '../hooks/useAuth.legacy';
...
// User Profile Card
const ProfileCard = ({ user, subscription, onLogout, addToast }) => {
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      {!subscription?.hasPaidAccess && (
        <Button variant="secondary" fullWidth onClick={() => window.open('https://useviola.com/pricing', '_blank')}>
...
const AuthForm = ({ onSuccess }) => {
  // 'login' | 'register' | 'magic' | 'forgot-password' | 'code' | 'mfa'
  ...
  const { login, verifyMfaTotp, register, requestMagicLink, startOAuthFlow, error, errorDetails, clearError } = useAuth();
  // 'code' = Path C2 — user types the 8-char short code from the magic-link
  // email instead of clicking the URL. 'magic' / 'forgot-password' / 'mfa'
  // don't need a password input either.
  ...
  const verifyShortCode = async () => {
    // Path C2: POST email + 8-char code to /auth/magic-link/verify-code.
    // On success the desktop hub mints a local session via cookie + a CSRF
    // token; the parent reload picks them up the same way the URL flow does.
    try {
      const response = await fetch('/auth/magic-link/verify-code', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        const msg =
          (typeof detail === 'object' && (detail.message || detail.detail)) ||
          (typeof detail === 'string' ? detail : null) ||
          "We couldn't sign you in with that code. Try a fresh code from your email.";
      ...
      return {
        success: false,
        error: err instanceof Error ? err.message : 'Network error verifying sign-in code.',
      };
    }
  };

  const requestPasswordReset = async (resetEmail) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.detail || "We couldn't send the reset link. Please try again.");
      }
      ...
      return {
        success: false,
        error: err instanceof Error ? err.message : "We couldn't send the reset link. Please try again.",
      };
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (mode === 'code') {
        ...
        // Second-factor TOTP step. mfaToken came from the prior login() call.
        if (!mfaToken) {
          setLocalError('Session expired — please sign in again.');
          switchMode('login');
        } else {
          result = await verifyMfaTotp(mfaToken, mfaCode.trim());
          if (result.success) {
            onSuccess?.();
          } else {
            setLocalError(result.error || "That code didn't match. Try again or use a backup code.");
        ...
        if (result.mfa_required && result.mfa_token) {
          // Stash the MFA token and switch to TOTP-entry mode. Don't clear
          // email so the heading stays informative.
  ...
  return (
    <div style={{ padding: '24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {mode === 'mfa' && (
            <div style={{
              ...
              Signing in as <span style={{ color: theme.colors.textPrimary }}>{email}</span>
            </div>
          )}
          ...
          {mode === 'mfa' && (
            <Input
              ...
              icon={Icons.Lock}
              aria-label="TOTP or backup code"
              autoComplete="one-time-code"
              inputMode="text"
              maxLength={16}
              autoFocus
              style={{ letterSpacing: '0.3em', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' }}
            />
          )}
      ...
      <div style={{
        ...
        {(mode === 'magic' || mode === 'forgot-password' || mode === 'code' || mode === 'mfa') && (
          <>
            {(mode === 'magic') && (
              <button
                type="button"
            ...
            >
              {mode === 'mfa' ? 'Cancel' : 'Back to sign in'}
            </button>
          </>
        )}
      </div>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
      ...
      <div>
        ...
        <div style={{ color: enabled ? theme.colors.statusGreen : theme.colors.textMuted, fontSize: '13px' }}>
          {enabled ? 'Synced across your devices' : 'Sign in to sync across your devices'}
        </div>
      </div>
    </div>
    <div style={{
...
//
// Uses the auth OAuth flow (system browser) instead of the consent popup flow.
// Google blocks sign-in from embedded browsers (RFC 8252), so the auth login
// flow is used — it already includes calendar scopes and stores tokens in the
// auth DB where CalendarManager reads them.
...
export const CalendarSettings = () => {
  const { startOAuthFlow, user } = useAuth();
  const { disconnect } = useCalendarProviders();
  ...
  }, [startOAuthFlow, checkCalendarStatus]);
...
export function AccountTab({
  ...
  const { user, subscription, loading, isLoggedIn, logout } = useAuth();
    ...
    return (
      <>
        <Section title="Account">
          <ProfileCard user={user} subscription={subscription} onLogout={logout} addToast={addToast} />
        </Section>
        ...
        <Section title="Sync & Devices">
          <SyncStatus enabled={!!subscription?.hasPaidAccess} />
        </Section>
